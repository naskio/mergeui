...
def test_list_models(model_repository):
    ...
    assert len(result) == 1
    result = model_repository.list_models(license_='cc-by-nc-4.0', architecture="NotFound")
    assert len(result) == 0
    # exclude
    result = model_repository.list_models(license_='apache-2.0')
    assert len(result) == 5
    result = model_repository.list_models(license_='apache-2.0', label="MergedModel")
    assert len(result) == 1
    result = model_repository.list_models(license_='apache-2.0', not_label="MergedModel")
    assert len(result) == 4
    # base_model
    result = model_repository.list_models(license_='apache-2.0', label="MergedModel",
    ...
    assert len(result) == 5
    # merge_method
    result = model_repository.list_models(merge_method='slerp')
    assert len(result) == 2
